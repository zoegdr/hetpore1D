...
% ----------------------------------
% 2     |         Perm
T{1,1} = cell(c,3); Phi{1,1} = cell(c,3); dP{1,1} = cell(c,3); k{1,1} = cell(c,3); S{1,1} = cell(c,3); U{1,1} = cell(c,3); Q{1,1} = cell(c,3); % Stiff, AL
T{1,2} = cell(c,3); Phi{1,2} = cell(c,3); dP{1,2} = cell(c,3); k{1,2} = cell(c,3); S{1,2} = cell(c,3); U{1,2} = cell(c,3); Q{1,2} = cell(c,3); % Stiff, AD
T{2,1} = cell(c,3); Phi{2,1} = cell(c,3); dP{2,1} = cell(c,3); k{2,1} = cell(c,3); S{2,1} = cell(c,3); U{2,1} = cell(c,3); Q{2,1} = cell(c,3); % Perm, AL
T{2,2} = cell(c,3); Phi{2,2} = cell(c,3); dP{2,2} = cell(c,3); k{2,2} = cell(c,3); S{2,2} = cell(c,3); U{2,2} = cell(c,3); Q{2,2} = cell(c,3); % Perm AD
...
% Solve varying d and l 
    ...
    % AL
        ...
        T{1,1}{1,1} = Ts; Phi{1,1}{1,1} = Phis; dP{1,1}{1,1} = dPs; k{1,1}{1,1} = ks; S{1,1}{1,1} = Ss; U{1,1}{1,1} = Uss; Q{1,1}{1,1} = Qs; % store solution, Stiff no damage
        T{2,1}{1,1} = Ts; Phi{2,1}{1,1} = Phis; dP{2,1}{1,1} = dPs; k{2,1}{1,1} = ks; S{2,1}{1,1} = Ss; U{2,1}{1,1} = Uss; Q{2,1}{1,1} = Qs; % Perm no damage
        metric_var_d{1,1}(1,:) = calc_metric(Phis,Zs,Ts,1); % the 1 here is the choice of metric. See function.
    ...
    % AD
        ...
        T{2,2}{1,1} = Ts; Phi{2,2}{1,1} = Phis; dP{2,2}{1,1} = dPs; k{2,2}{1,1} = ks; S{2,2}{1,1} = Ss; U{2,2}{1,1} = Uss; Q{2,2}{1,1} = Qs;
     ...
     % Vary d and l       
        for i = 2:c
            ...
            for j = 1:3
                ...
                % AL
                    params.Astar = 0.2;
                    [params,Ts,~,Phis,~,Uss,~,Qs,Ss,~,~,dPs,ks] = tendon_uniaxial_cyclic_load_stressdiff(params);
                    T{1,1}{i,j} = Ts; Phi{1,1}{i,j} = Phis; dP{1,1}{i,j} = dPs; k{1,1}{i,j} = ks; S{1,1}{i,j} = Ss; U{1,1}{i,j} = Uss; Q{1,1}{i,j} = Qs;
            ...
            for j = 1:3
                ...
                % AL
                    ...
                    T{2,1}{i,j} = Ts; Phi{2,1}{i,j} = Phis; dP{2,1}{i,j} = dPs; k{2,1}{i,j} = ks; S{2,1}{i,j} = Ss; U{2,1}{i,j} = Uss; Q{2,1}{i,j} = Qs;
                ...
                % AD
                    ...
                    if Ts(end) < params.p*2*pi/params.omega
                        ...
                        T{2,2}{i,j} = Ts; Phi{2,2}{i,j} = Phis; dP{2,2}{i,j} = dPs; k{2,2}{i,j} = ks; S{2,2}{i,j} = Ss; U{2,2}{i,j} = Uss; Q{2,2}{i,j} = Qs;
