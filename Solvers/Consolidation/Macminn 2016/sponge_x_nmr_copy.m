function [xss,ts,nss,uss,pss,sigss,qs,dps,as] = sponge_x_nmr_copy(spongepar)
    % Scaling:
    %   n is porosity
    %   x is x/L
    %   t is t/T
    ...
    % else
        ...
        elseif strcmp(stress_law,'neo')
            sig = @(n) 0.5*M0*(J(n)-1./(J(n)))+0.5*L0*(J(n)+1./(J(n))-2); % Neo-Hookean elasticity
            n_from_sig = @(s) -(1-n0)*(M0+L0)./ ( L0+s + sqrt((L0+s).^2 + (M0+L0)*(M0-L0)) ) + 1;
        else
