function [params,Ts,Zs,Phis,Uss,Ss,Ps,Qs,dUdZ,dPs,ks] = cylic_uniaxial_Lag_disp(params)
    ...
    %   * T is time
    %   * Z is space
    %   * Phi is porosity (NOT normalised)
    ...
    %   The lower edge is impermeable Vs=Vf=Us=0
    %
    % METHOD
...
[Uss,Ps,Qs,Ss,dUdZ,dPs] = main_ode_post(Ts,Phis);
ks = k(Phis,Zs);
    ...
    function [Uss,Ps,Qs,Ss,dUdZ,dPs] = main_ode_post(Ts,Phis)
...
function status = ode_check_save(T,Y,flag,params)
    ...
    if strcmp(flag,'')
        ...
        u_err = us(end)/(1-a);
        if abs(u_err)>1E-5
            disp(['*** WARNING: rel err in displacement at right boundary = ' num2str(u_err)]) % doesnt work because comparing a at boundary and u at dZ/2
            status = 1; % stop
        else
