function [params,T,Zs,Z,Phis,Ws,Wss,P,Q,S,dWdZ,Fluxes,Adv,Diff] = tendon_uniaxial_cyclic_load_COPY(params)
...
% WORKING 11/07/2024 13:47
    ...
    Lambda = @(Z) nu/(1-nu) * M(Z);
    dLambda = @(Z) nu/(1-nu) * dM(Z);

    % Define a function for diffusivity
        if strcmp(params.stress_law,'neo')
            D = @(Phi,Z) 0.5*k(Phi,Z)./(1+Phi)*(M(Z)+Lambda(Z)+(M(Z)-Lambda(Z))./(1+Phi)^2); % Neo-Hookean elasticity
    ...
    % Define a function for A (extra advection term from heterogenous stiffness)
        if strcmp(params.stress_law,'neo')
            A = @(Phi,Z) 0.5*k(Phi,Z)./(1+Phi)*(dM(Z)*(1+Phi-1./(1+Phi))+dLambda(Z)*(1+Phi+1./(1+Phi)-2)); % Neo-Hookean elasticity
        elseif strcmp(params.stress_law,'linear')
   ...
   options = odeset('RelTol',1E-5, 'AbsTol',1E-5);
    ...
    [Z,Ws,Wss,P,Q,S,dWdZ] = main_ode_post(T,Phis);
    ...
    function Phidots = odefun(T,Phis)
        ...
        S_star = 0.5*A_star*(1-cos(omega*T)); 
        Phi_star = (Lambda(Zs(N)+dZ/2)+S_star+sqrt((Lambda(Zs(N)+dZ/2)+S_star)^2+(M(Zs(N)+dZ/2)+Lambda(Zs(N)+dZ/2))*(M(Zs(N)+dZ/2)-Lambda(Zs(N)+dZ/2))))/(M(Zs(N)+dZ/2)+Lambda(Zs(N)+dZ/2)) - 1;
        ...
        for i = 2:N
            F_left(1,i) = -A((Phis(i)+Phis(i-1))/2,Zs(i)-dZ/2) - D((Phis(i)+Phis(i-1))/2,Zs(i)-dZ/2)*(Phis(i)-Phis(i-1))/dZ;
            F_right(1,i) = -A((Phis(i)+Phis(i-1))/2,Zs(i)-dZ/2) - D((Phis(i)+Phis(i-1))/2,Zs(i)-dZ/2)*(Phis(i)-Phis(i-1))/dZ;
        end
        ...
        F_right(1,N+1) = -A(Phi_star,Zs(N)+dZ/2) - D(Phi_star,Zs(N)+dZ/2)*2*(Phi_star-Phis(N))/dZ; % = -adot (Vs=adot F(N+1)=-Vs but would need to calculate adot...)
    ...
    function [Z,Ws,Wss,P,Q,S,dWdZ] = main_ode_post(T,Phis)
        ...
        for n = 1:length(T)
            ...
            Phi_star(n) = (Lambda(Zs(N)+dZ/2)+S_star(n)+sqrt((Lambda(Zs(N)+dZ/2)+S_star(n))^2+(M(Zs(N)+dZ/2)+Lambda(Zs(N)+dZ/2))*(M(Zs(N)+dZ/2)-Lambda(Zs(N)+dZ/2))))/(M(Zs(N)+dZ/2)+Lambda(Zs(N)+dZ/2))-1;
            ...
            for j = 1:N-1
                dP_inner_walls(n,j) = - 0.5* (((Lambda(Zs(j+1))+M(Zs(j+1)))*(1+Phis(n,j+1))+(Lambda(Zs(j+1))-M(Zs(j+1)))./(1+Phis(n,j+1))-2*Lambda(Zs(j+1)))...
                -((Lambda(Zs(j))+M(Zs(j)))*(1+Phis(n,j))+(Lambda(Zs(j))-M(Zs(j)))./(1+Phis(n,j))-2*Lambda(Zs(j))))/dZ;
            end
            % Add boundary values. Ghost points P_0 = P_1 ; P_N+1 = 2*Phi_star - P_N
            dP_walls(n,:) = [0,dP_inner_walls(n,:),...
            -0.5*(((Lambda(Zs(N)+dZ/2)+M(Zs(N)+dZ/2))*(1+2*Phi_star(n)-Phis(n,N))+(Lambda(Zs(N)+dZ/2)-M(Zs(N)+dZ/2))./(1+2*Phi_star(n)-Phis(n,N))-2*Lambda(Zs(N)+dZ/2))...
            - ((Lambda(Zs(N))+M(Zs(N)))*(1+Phis(n,N))+(Lambda(Zs(N))-M(Zs(N)))./(1+Phis(n,N))-2*Lambda(Zs(N))))/dZ];
            ...
            % dP/dZ = d/dZ(S), P(1)=0 
            % => P(Z) = S(Z)-S_star
            P(n,s) = 0.5*((Lambda(Zs(s))+M(Zs(s)))*(1+Phis(n,s))+(Lambda(Zs(s))-M(Zs(s)))./(1+Phis(n,s))-2*Lambda(Zs(s)))...
                - 0.5*((Lambda(Zs(end)+dZ/2)+M(Zs(end)+dZ/2))*(1+Phi_star(n))+(Lambda(Zs(end)+dZ/2)-M(Zs(end)+dZ/2))./(1+Phi_star(n))-2*Lambda(Zs(end)+dZ/2));
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                S(n,s) = .5*M(Zs(s)).*(1+Phis(n,s)-1./(1+Phis(n,s)))+.5*Lambda(Zs(s)).*(Phis(n,s)+1./(Phis(n,s)+1)-2);
    ...
    function [Adv,Diff] = calc_adv_diff_coeffs(Phis,Zs)
        for j=1:size(Phis,1)
            for i=1:N
                Adv(j,i) = A(Phis(j,i),Zs(i));
                Diff(j,i) = D(Phis(j,i),Zs(i));
            end
        end
    end
