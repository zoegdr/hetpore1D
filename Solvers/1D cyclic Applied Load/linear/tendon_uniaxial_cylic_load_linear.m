function [params,t,zs,z,phis,ws,wss,ps,qs,sigs,dwdz,fluxes,adv,diff] = tendon_uniaxial_cylic_load_linear(params)
...
% Define a function for permeability

k = @(phi,z) 1-dp*exp(-(z-lp)^2/(2*(1/16)^2));
...
function phidots = odefun(t,phis)
      ...
      % Calculate stress
        ...
        for i = 2:N
            F_left(1,i) = -A((phis(i)+phis(i-1))/2,zs(i)-dz/2)-k(phis(i),zs(i)-dz/2)*M(zs(i)-dz/2)*(phis(i)-phis(i-1))/dz;
            F_right(1,i) = -A((phis(i)+phis(i-1))/2,zs(i)-dz/2)-k(phis(i),zs(i)-dz/2)*M(zs(i)-dz/2)*(phis(i)-phis(i-1))/dz;
        end
        ...
        F_left(1,1) = -A(phi_star,zs(1)-dz/2)-M(zs(1)-dz/2)*2*(phis(1)-phi_star)/dz;
        F_right(1,N+1) = -A(phis(1),zs(end)+dz/2);
...
function [z,ws,wss,p,q,sig,dwdz] = main_ode_post(t,phis)
    ...
    for n = 1:length(t)
        ...
        for j = 1:N-1
            dp_inner_walls(n,j) = -(M(zs(j+1))*phis(n,j+1)-M(zs(j))*phis(n,j))/dz;
        end
        % Add boundary values. Ghost points P_0 = P_1 ; P_N+1 = 2*phi_star - P_N
        dp_walls(n,:) = [ -( M(zs(1)-dz/2 )*2*( phis(n,1)-phi_star(n) ))/dz , dp_inner_walls(n,:) , 0 ];
        ...
        q(n,s) = k(phis(n,s),zs(s))*dp_(n,s)/(1+phis(n,s));
        ...
        % dp/dz = d/dz(s), p(1)=0 
        % => p(z) = s(z)-S_star
        p(n,s) = M(zs(s))*phis(n,s)-M(zs(end)+dz/2)*phi_star(n);
...
function [adv,diff] = calc_adv_diff_coeffs(phis,zs)
    for j=1:size(phis,1)
        for i=1:N
            ...
            diff(j,i) = k(phis(j,i),zs(i))*M(zs(i));
        end
    end
end
