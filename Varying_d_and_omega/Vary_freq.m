...
function metric_var_freq = Vary_freq
...
% ----------------------------------
% 2     |         Perm
T{1,1} = cell(c,4); Phi{1,1} = cell(c,4); k{1,1} = cell(c,4); S{1,1} = cell(c,4); U{1,1} = cell(c,4); Q{1,1} = cell(c,4); % Stiff, AL
T{1,2} = cell(c,4); Phi{1,2} = cell(c,4); k{1,2} = cell(c,4); S{1,2} = cell(c,4); U{1,2} = cell(c,4); Q{1,2} = cell(c,4); % Stiff, AD
T{2,1} = cell(c,4); Phi{2,1} = cell(c,4); k{2,1} = cell(c,4); S{2,1} = cell(c,4); U{2,1} = cell(c,4); Q{2,1} = cell(c,4); % Perm, AL
T{2,2} = cell(c,4); Phi{2,2} = cell(c,4); k{2,2} = cell(c,4); S{2,2} = cell(c,4); U{2,2} = cell(c,4); Q{2,2} = cell(c,4); % Perm AD
...
T{1,1}{1,1} = Ts; Phi{1,1}{1,1} = Phis; k{1,1}{1,1} = ks; S{1,1}{1,1} = Ss; U{1,1}{1,1} = Uss; Q{1,1}{1,1} = Qs;
...
params.Astar = 0.2;
[params,Ts,Zs,Phis,Uss,Ss,~,Qs,~,ks] = cyclic_uniaxial_stress(params);
T{1,1}{i,j} = Ts; Phi{1,1}{i,j} = Phis; k{1,1}{i,j} = ks; S{1,1}{i,j} = Ss; U{1,1}{i,j} = Uss; Q{1,1}{i,j} = Qs; % store solution
...
params.Astar = 0.2;
[params,Ts,Zs,Phis,Uss,Ss,~,Qs,~,ks] = cyclic_uniaxial_stress(params);
T{2,1}{i,j} = Ts; Phi{2,1}{i,j} = Phis; k{2,1}{i,j} = ks; S{2,1}{i,j} = Ss; U{2,1}{i,j} = Uss; Q{2,1}{i,j} = Qs; % store solution
...
if Ts(end) < params.p*2*pi/params.omega % if code breaks assign NaN
   T{2,2}{i,j} = NaN; Phi{2,2}{i,j} = NaN; k{2,2}{i,j} = NaN; S{2,2}{i,j} = NaN; U{2,2}{i,j} = NaN; Q{2,2}{i,j} = NaN;
    ...
    T{2,2}{i,j} = Ts; Phi{2,2}{i,j} = Phis; k{2,2}{i,j} = ks; S{2,2}{i,j} = Ss; U{2,2}{i,j} = Uss; Q{2,2}{i,j} = Qs; % store solution
end
